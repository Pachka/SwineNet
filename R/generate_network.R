...
<<<<<<<< HEAD:R/get_network.R
#' @param splitByEdges FIX ME - A vector or characters including one or several time periods among year, semester and month. FALSE is leading to the production of the complete network.
#' @param splitByVertex FIX ME - A vector or characters including one or several time periods among year, semester and month. FALSE is leading to the production of the complete network.
========
...
>>>>>>>> 42f1e3243b0b136dd72c76edb43e40e46dfee4bb:R/generate_network.R
...
<<<<<<<< HEAD:R/get_network.R
  if (!inherits(sites, "data.frame") | FALSE %in% (c(movements$from, movements$to) %in% sites[,1]))
    stop("sites object should be a data.frame where the first column contains all ids in from and to columns of movement object.")
========
...
<<<<<<<< HEAD:R/get_network.R
    stop("splitByVertex object should be vector of characters or logical values.")
  
  if(splitByEdges %>% .[!is.na(.)] %>% match(., colnames(movements)) %>% anyNA)
    stop("splitByEdges object should be refering to a column name of the movements data.frame.")
  if(splitByVertex %>% .[!is.na(.)] %>% match(., colnames(sites)) %>% anyNA)
    stop("splitByVertex object should be refering to a column name of the sites data.frame.")
========
    stop("splitByEdges object should be vector of characters or logical values.")
...
<<<<<<<< HEAD:R/get_network.R
   sitesInNetwork <- sapply(splitByVertex, function(x){
     if(is.na(x)){
       sites %<>% list
     names(sites)  <- "all"}
     if(!is.na(x))
   ...
   if(splitByVertex %>% length > 1)
     sitesInNetwork %<>>% unlist(.,recursive=FALSE)
========
...
<<<<<<<< HEAD:R/get_network.R
   if(is.null(names(sitesInNetwork))) names(sitesInNetwork) <-  splitByVertex %>% paste0(.,".") %>% paste0(., (sitesInNetwork %>% dimnames %>% .[[1]]))
   
   names(movInNetwork)[names(movInNetwork) == "NA.all"] <- "all"
   names(sitesInNetwork)[names(sitesInNetwork) == "NA.all"] <- "all"
   
  # Generate network(s)
   
   experiementationPlan <- expand.grid(movInNetwork %>% names, sitesInNetwork%>% names) 
...
<<<<<<<< HEAD:R/get_network.R
     # Select movements based on selected sites
     vertex <- sitesInNetwork[[experiment["Var2"]]][,1]
========
...
>>>>>>>> 42f1e3243b0b136dd72c76edb43e40e46dfee4bb:R/generate_network.R
     ...
     g <- igraph::graph_from_data_frame(d = movInNetwork[[experiment["Var1"]]],
                                   directed = T,
<<<<<<<< HEAD:R/get_network.R
                                   vertices = sitesInNetwork[[experiment["Var2"]]])
========
